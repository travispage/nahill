...
function wpgrade_callback_load_woocommerce_assets(){
	...
	wp_enqueue_style('wpgrade-woocommerce', get_template_directory_uri() . '/woocommerce.css', array('woocommerce-general'), wpgrade::cachebust_string(wpgrade::themefilepath('woocommerce.css')) );
}
...
if ( ! function_exists( 'woocommerce_template_loop_product_thumbnail' ) ) {
	...
	function woocommerce_template_loop_product_thumbnail() {
		...
		echo '<span class="masonry__item-image">' . woocommerce_get_product_thumbnail('medium-size');
...
function wpgrade_filter_product_categories($term_links){
	...
	if ( !empty($term_links) ) {
		foreach($term_links as &$link){
			$link = str_replace('<a ', '<a class="btn-link" ', $link);
		}
	}
...
function wpgrade_woocommerce_breadcrumbs() {
	return array(
		...
		'before'      => '',
		'after'       => '',
		'home'        => _x( 'Shop', 'breadcrumb', wpgrade::textdomain() )
	);
}
