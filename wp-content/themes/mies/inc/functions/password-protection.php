...
function wpgrade_callback_the_password_form( $form ) {
	...
	$form   = '<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" method="post">
		<p>' . __( "This post is password protected. To view it please enter your password below:", 'mies_txtd' ) . '</p>
		<div class="row">
			<div class="column  span-12  hand-span-10">
				<input name="post_password" id="' . $label . '" type="password" size="20" placeholder="' . __( "Password", 'mies_txtd' ) . '"/>
			</div>
			<div class="column  span-12  hand-span-2">
				<input type="submit" name="Access" value="' . esc_attr__( "Access", 'mies_txtd' ) . '" class="btn post-password-submit"/>
			</div>
		</div>
	</form>';
	...
	if ( ! $hasher->CheckPassword( $post->post_password, $hash ) ) {
		...
		$msg  = '<span class="wrong-password-message">' . __( 'Sorry, your password did not match', 'mies_txtd' ) . '</span>';
		$form = $msg . $form;
	}
